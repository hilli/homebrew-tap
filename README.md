...
The files in `Casks/` are generated by the upstream projects' GoReleaser release workflows. Tap-only changes can be overwritten by the next release, so the upstream generators/templates must also emit the current [Homebrew cask DSL](https://docs.brew.sh/Cask-Cookbook#stanza-flight_steps) before publishing.

Omit the deprecated `url` parameter `verified:` and use Homebrew's default URL verification. Post-install commands must use `postflight_steps`, `run`, and install-time `{{...}}` tokens instead of legacy `postflight` blocks, `system_command`, and Ruby path interpolation. Preserve the macOS-only scope of the existing quarantine-removal command:

```ruby
postflight_steps do
  on_macos do
    run "/usr/bin/xattr", args: ["-dr", "com.apple.quarantine", "{{staged_path}}/BINARY_NAME"]
  end
end
```
